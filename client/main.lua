        ...
        Wait(1)
            if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
                ...
                if driverPed == GetPlayerPed(-1) then
                    if speed > 160 and not IsPedInAnyHeli(GetPlayerPed(-1)) then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and Config.GunshotAlert then
			...
			if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("weapon_snowball") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_PUMPSHOTGUN") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_STUNGUN") then 
				Citizen.Wait(1000)
			else
				Citizen.Wait(3000)
				if (isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted then
...
-- 			if IsPedInMeleeCombat(GetPlayerPed(-1)) and not origin3 and nearNpc then 
-- 					Wait(2000)
-- 					origin3 = true
-- 					local currentVehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
-- 					local plyPos = GetEntityCoords(GetPlayerPed(-1))
...
-- 						if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_BAT") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_KNUCKLEDUSTER") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_NIGHTSTICK") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_GOLFCLUB") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_FLASHLIGHT") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_POOLCLUE") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_WRENCH") then
-- 							weapon = 1
...
-- 						elseif GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_KNIFE") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_SWITCHBLADE") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_CROWBAR") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_BOTTLE") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_DAGGER") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_HATCHET") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_MACHETE") or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey("WEAPON_BATTLEAXE") then
-- 							weapon = 2
-- 							TriggerServerEvent('esx_outlawalert:combatInProgress', {
-- 								x = ESX.Math.Round(playerCoords.x, 1),
-- 								y = ESX.Math.Round(playerCoords.y, 1),
